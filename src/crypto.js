// crypto.js - Commit-reveal helpers: nonce generation, SHA-256, XOR
...
export function xorNonces(hexA, hexB) {
  var a = parseInt(hexA, 16);
  var b = parseInt(hexB, 16);
  return (a ^ b) >>> 0;
}

// Pure-JS synchronous SHA-256 (needed because the reducer must be synchronous)
...
export function sha256sync(bytes) {
  ...
  padded.set(bytes);
  padded[len] = 0x80;
  // Big-endian 64-bit length at the end (only lower 32 bits needed for small inputs)
